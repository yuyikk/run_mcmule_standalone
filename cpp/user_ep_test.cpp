...
void CallGeant4(double **res, double *p1, double *p2, double *p3, double *p4, double *p5, double *p6, double *p7);

int c = 0;
int pre_call_counter = 1;
bool in_pre_mode = true;
double theta_mc;
...
extern "C"
{
    ...
    double mcmule_lower_bounds[2] = {0.5};
    double mcmule_upper_bounds[2] = {8};
    ...
    double get_theta(const double *p1, const double *p2)
    {
        ...
        if (mag1 == 0 || mag2 == 0)
        {
            std::cerr << "Warning: zero-length momentum vector!" << std::endl;
            return 0.;
        }
    ...
    void mcmule_user_initialisation(void)
    {
        ...
        // Get the SDManager and EventAction
        // reconstructed kinematics and decision of accepting an event
        // are provided by the EventAction
        ...
        {
            std::cerr << "We are running e-p scattering with Ebeam (MeV) = " << Ebeam << std::endl;
            s = Mel * Mel + Mproton * Mproton + 2 * Mproton * Ebeam;
            std::cerr << "Please choose proton form factor models:\n";
            std::cerr << "  0: dipole form factor.\n";
            std::cerr << "  1: monopole form factor.\n";
            std::cerr << "  2: Kelly form factor.\n";
            std::cerr << "  3: Rational(1, 1) form factor.\n";
            int user_input = fortran_read_int();
            // std::cin >> user_input;
            if (user_input == 0)
            {
                std::cerr << "Using McMule's default dipole form factors" << std::endl;
                std::cerr << "with lambda = " << mcmule_protonff_lambda << " and kappa = " << mcmule_protonff_kappa << std::endl;
                mcmule_protonff = &__nucl_protonff_MOD_proton_dipole;
            }
            else if (user_input == 1)
            {
                std::cerr << "Using McMule's default monopole form factors" << std::endl;
                std::cerr << "with lambda = " << mcmule_protonff_lambda << " and kappa = " << mcmule_protonff_kappa << std::endl;
                mcmule_protonff = &__nucl_protonff_MOD_proton_monopole;
            }
            else if (user_input == 2)
            {
                std::cerr << "Using Kelly form factor" << std::endl;
                mcmule_protonff = &kelly_proton_ff;
            }
            else if (user_input == 3)
            {
                std::cerr << "Please enter parameter R(adius): " << std::endl;
                rational11R = fortran_read_real();
                std::cerr << "Please enter parameter P: " << std::endl;
                rational11P = fortran_read_real();
                std::cerr << "Using Rational(1,1) form factor" << std::endl;
                std::cerr << "with p = " << rational11P << " and R = " << rational11R << std::endl;
    ...
    void mcmule_measurement_function(double **res, double *p1, double *p2, double *p3, double *p4, double *p5, double *p6, double *p7)
    {
        ...
        double th_l = get_theta(p1, p3) * rad2deg;
        res[0][0] = th_l;
...
// void CallGeant4(double **res, double *p1, double *p2, double *p3, double *p4, double *p5, double *p6, double *p7)
// {
//     primaryGenAction->AddParticle("e-", p3[0], p3[1], p3[2], p3[3]);
//     if (__global_def_MOD_which_piece[1] == 'e')
//     {
//         primaryGenAction->AddParticle("e-", p4[0], p4[1], p4[2], p4[3]);
//     }
//     else if (__global_def_MOD_which_piece[1] == 'p')
//     {
//         primaryGenAction->AddParticle("proton", p4[0], p4[1], p4[2], p4[3]);
//     }

//     primaryGenAction->AddParticle("gamma", p5[0], p5[1], p5[2], p5[3]);
//     primaryGenAction->AddParticle("gamma", p6[0], p6[1], p6[2], p6[3]);
//     primaryGenAction->AddParticle("gamma", p7[0], p7[1], p7[2], p7[3]);

//     // Execute Geant4
//     runManager->BeamOn(1);
//     G4bool AcceptEvent = MyEventAction->IsEventAccepted();
//     G4int event_type = MyEventAction->GetEventType();

//     G4double theta_rec = MyEventAction->GetThetaEP();
//     // G4cout << "Theta: " << theta << G4endl;
//     res[0][0] = theta_mc;
//     res[0][1] = theta_rec;
//     mcmule_pass_cut[0] = (theta_mc > mcmule_lower_bounds[0] && theta_mc < mcmule_upper_bounds[0]);
//     mcmule_pass_cut[1] = (theta_rec > mcmule_lower_bounds[1] && theta_rec < mcmule_upper_bounds[1]) && AcceptEvent;
...
//                   << std::setw(12) << p4[1] << std::setw(12) << p4[2]
//                   << std::setw(12) << p4[3] << std::setw(12) << std::sqrt(dot4D(p4, p4)) << std::endl;
//         std::cout << std::left << std::setw(15) << "theta_mc: " << std::setw(12) << theta_mc << std::endl;
//         std::cout << std::left << std::setw(15) << "theta_rec: " << std::setw(12) << theta_rec << std::endl;
//         std::cout << "== == == == == == == == == == == == == == == == == == ==" << std::endl;
//         c++;
//     }
// }
